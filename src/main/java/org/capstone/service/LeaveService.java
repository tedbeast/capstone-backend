...
//        }
    ...
    public List<Leave> getAllLeaveByEmployeeId(int employeeID) throws LeaveException {
        ...
        List <Leave> l = employee.getLeave();
        System.out.println("Leaves: " +l);
        if (l.isEmpty()) {
...
//        }
        ...
        if (employee.getLeave() == null) {
            employee.setLeave(new ArrayList<>());
        }
        employee.getLeave().add(savedLeave);
    ...
    public List<Leave> findAllLeavesByManagerId(int managerId) {
        Optional<Manager> mangerO = managerRepository.findById(managerId);
